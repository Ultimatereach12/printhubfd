...
if(!function_exists('s7upf_list_post_type')){
    function s7upf_list_post_type($post_type = 'page',$type = true){
        ...
        if($type){
            $page_list[] = array(
                'value' => '',
                'label' => esc_html__('-- Choose One --','druck')
            );
        }
        else $page_list[] = esc_html__('-- Choose One --','druck');
        if(is_admin()){
...
//VC list sidebar
if(!function_exists('s7upf_vc_sidebar_ids')){
    function s7upf_vc_sidebar_ids(){
        global $wp_registered_sidebars;
        $r=array();
        $r[]=esc_html__('--Select--','druck');
        if(!empty($wp_registered_sidebars)){
...
if(!function_exists('s7upf_get_order_list')){
    function s7upf_get_order_list($current=false,$extra=array(),$return='array'){
        $default = array(
            esc_html__('None','druck')               => 'none',
            esc_html__('ID','druck')                 => 'ID',
            esc_html__('Author','druck')             => 'author',
            esc_html__('Title','druck')              => 'title',
            esc_html__('Name','druck')               => 'name',
            esc_html__('Date','druck')               => 'date',
            esc_html__('Last Modified Date','druck') => 'modified',
            esc_html__('Post Parent','druck')        => 'parent',
        );
...
if(!function_exists('s7upf_breadcrumb')){
    function s7upf_breadcrumb($step = '') {
        global $post;
        if(is_home() && !is_front_page()) echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','druck').'</a>'.$step.'<span>'.esc_html__('Blog','druck').'</span>';
        else echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','druck').'</a>';
        if (is_single()){
        ...
        elseif(is_search()) echo "<span>".esc_html__("Search Results for: ","druck").get_search_query().'</span>';
        elseif(is_404()) echo "<span>".esc_html__("404 ","druck")."</span>";
    }
}
//Don't Show popup
...
if(!function_exists('s7upf_display_metabox')){
    function s7upf_display_metabox($type =''){
        switch ($type) {
            ...
            case 'social':
				 ?>
                <ul class="social-meta-data title18 list-inline-block">
					<li><a class="grow black" target="_blank" href="<?php echo esc_url('https://plus.google.com/share?url='.get_the_permalink());?>"><i class="la la-google-plus"></i></a></li>
					<li><a class="grow black" target="_blank" href="<?php echo esc_url('http://pinterest.com/pin/create/button/?url='.get_the_permalink().'&amp;media='.wp_get_attachment_url(get_post_thumbnail_id()));?>"><i class="la la-pinterest-p"></i></a></li>
					<li><a class="grow black" target="_blank" href="<?php echo esc_url('http://www.twitter.com/share?url='.get_the_permalink());?>"><i class="la la-twitter"></i></a></li>
					<li><a class="grow black" target="_blank" href="<?php echo esc_url('http://www.facebook.com/sharer.php?u='.get_the_permalink());?>"><i class="la la-facebook"></i></a></li>
					<li><a class="grow black" target="_blank" href="mailto:?subject='<?php echo esc_attr__("I wanted you to see this site&amp;body=Check out this site","druck");?>' '<?php echo esc_url(get_the_permalink())?>'"><i class="la la-envelope"></i></a></li>
					<li><a class="grow black" target="_blank" href="<?php echo esc_url('https://www.linkedin.com/cws/share?url='.get_the_permalink());?>"><i class="la la-linkedin"></i></a></li>
                </ul>               
                <?php
                break;
            ...
            case 'social2':
                ...
                <div class="social-meta-data flex-wrapper align_items-center justify_content-space-between">
                    <div class="post-meta-data data-user"><i aria-hidden="true" class="fa fa-comment-o silver"></i>
					...
					<div class="data-social">
						<a target="_blank" href="<?php echo esc_url('http://www.facebook.com/sharer.php?u='.get_the_permalink());?>"><i class="fa fa-facebook"></i></a>
						<a target="_blank" href="<?php echo esc_url('http://www.twitter.com/share?url='.get_the_permalink());?>"><i class="fa fa-twitter"></i></a>
						<a target="_blank" href="<?php echo esc_url('http://pinterest.com/pin/create/button/?url='.get_the_permalink().'&amp;media='.wp_get_attachment_url(get_post_thumbnail_id()));?>"><i class="fa fa-pinterest-p"></i></a>
						<a target="_blank" href="<?php echo esc_url('https://plus.google.com/share?url='.get_the_permalink());?>"><i class="fa fa-google-plus"></i></a>
					</div>
                </div>               
                <?php
                break;
            ...
            case 'single':
                ...
                <ul class="list-inline-block  post-meta-data">
                    <li><i class="title18 la la-calendar color"></i><span class="silver"><?php echo get_the_date()?></span></li>
                    <li><i class="title18 la la-user color"></i><a class="silver" href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>"><?php echo get_the_author(); ?></a></li>
                </ul>               
                <?php
                break;
            ...
            case 'grid':
                ...
                <ul class="list-inline-block title12 post-meta-data">
                    <li><i class="fa fa-calendar-o silver"></i><span class="silver"><?php echo get_the_date('M j Y')?></span></li>
                    <li><i aria-hidden="true" class="fa fa-comment-o silver"></i>
                        <a href="<?php echo esc_url( get_comments_link() ); ?>"> 
                        <span class="silver">
						<?php 
							echo get_comments_number();
			...
			case 'info':
				...
				?>
                <ul class="list-inline-block post-meta-data">
                    <li><span class="color"><?php echo get_the_date('M j.Y')?></span></li>
                    <li>
                        <a href="<?php echo get_category_link($cat_id);?>" class="silver"><?php echo get_cat_name($cat_id);?></a>
                    </li>
                </ul>               
                <?php endif;
				break;
            ...
            default:
                ?>
                <ul class="list-inline-block title12 post-meta-data">
                    <li><i class="fa fa-user silver" aria-hidden="true"></i><a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>"><?php echo get_the_author(); ?></a></li>
                    <li><i class="fa fa-calendar-o silver"></i><span class="silver"><?php echo get_the_date()?></span></li>
                    <li><i aria-hidden="true" class="fa fa-comment-o silver"></i>
                        <a href="<?php echo esc_url( get_comments_link() ); ?>"> 
                        <span class="silver">
						<?php 
                    ...
                    <?php 
                        ...
                        if($cats):?>
                        <li><i class="fa fa-folder-open silver" aria-hidden="true"></i>                            
                    ...
                    <?php 
                        ...
                        if($tags):?>
                        <li><i class="fa fa-tags silver" aria-hidden="true"></i>
...
if(!function_exists('s7upf_get_main_class')){
    function s7upf_get_main_class(){
        ...
        $main_class = 'content-wrap content-sidebar-none col-md-12 col-sm-12 col-xs-12';
...
if ( ! function_exists( 's7upf_get_google_link' ) ) {
    function s7upf_get_google_link() {
        ...
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'druck' ) ) {
...
if(!function_exists('s7upf_list_taxonomy'))
{
    ...
    {
        if($show_all) $list = array( esc_html__('--Select--','druck') => '' );
        else $list = array();
...
if(!function_exists('s7upf_check_enqueue')){
    function s7upf_check_enqueue($elements,$content2=''){
        global $post;
...
if(!function_exists('s7upf_get_template_portfolio')){
    function s7upf_get_template_portfolio( $view_name,$slug=false,$data=array(),$echo=FALSE ){
        $view_name = 'portfolio/'.$view_name;
        $html = S7upf_Template::load_view($view_name,$slug,$data,$echo);
        if(!$echo) return $html;
    }
}
...
if(!function_exists('s7upf_get_post_style')){
    function s7upf_get_post_style($style = 'element'){
        $list = apply_filters('s7upf_post_item_style',array(
            ...
            esc_html__('Info','druck')      => 'info',
            esc_html__('Table','druck')      => 'table',
            esc_html__('Inner','druck')      => 'inner',
            esc_html__('Simple','druck')      => 'simple',
            ));
        if($style != 'element'){
            $temp = array();
...
if(!function_exists('s7upf_get_post_list_style')){
    function s7upf_get_post_list_style($style = 'element'){
        $list = apply_filters('s7upf_post_list_item_style',array(
            esc_html__('Default','druck')      => '',
            esc_html__('Post list 2','druck')      => 'style2',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_product_list_style')){
    function s7upf_get_product_list_style($style = 'element'){
        $list = apply_filters('s7upf_product_list_item_style',array(
            esc_html__('Default','druck')      => '',
            esc_html__('Product list 2','druck')      => 'style2',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_product_style')){
    function s7upf_get_product_style($style = 'element'){
        $list = apply_filters('s7upf_product_item_style',array(
            esc_html__('Default','druck')           => '',
            esc_html__('Product Inner','druck')     => 'inner',
            esc_html__('Product Center','druck')    => 'center',
            esc_html__('Product Table','druck')    => 'table',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_portfolio_style')){
    function s7upf_get_portfolio_style($style = 'element'){
        $list = apply_filters('s7upf_portfolio_item_style',array(
            esc_html__('Default','druck')      => '',
            esc_html__('Hover Direction','druck')      => 'direction',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_portfolio_list_style')){
    function s7upf_get_portfolio_list_style($style = 'element'){
        $list = apply_filters('s7upf_portfolio_list_item_style',array(
            esc_html__('Default','druck')      => '',
            esc_html__('Portfolio list 2','druck')      => 'style2',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
...
if(!function_exists('s7upf_get_product_thumb_animation')){
    function s7upf_get_product_thumb_animation($style = 'element'){
        $list = apply_filters('s7upf_product_item_style',array(
            ...
            esc_html__('Zoom','druck')        => 'zoom-thumb',
            esc_html__('Rotate','druck')      => 'rotate-thumb',
            esc_html__('Zoom Out','druck')    => 'zoomout-thumb',
            esc_html__('Translate','druck')   => 'translate-thumb',
            esc_html__('Fade','druck')   => 'fade-thumb',
            ));
        if($style != 'element'){
            $temp = array();
            foreach ($list as $key => $value) {
                ...
                switch ($preload_style) {
                    ...
                    case 'custom-image':
                        ...
                        <div id="loading-center-absolute-image">
                            <img src="<?php echo esc_url($preload_img)?>" alt="<?php esc_attr_e("preload-image","druck");?>"/>
                        </div>
                        <?php
                        break;
...
if(!function_exists('s7upf_compare_url')){
    function s7upf_compare_url($icon='',$id = false,$text = '',$class='black',$title = ''){
        $html = '';
        if(empty($text)) $text = esc_html__("Compare","druck");
        if(empty($icon)) $icon = '<i class="la la-random"></i>';
        if(class_exists('YITH_Woocompare')){
            if(!$id) $id = get_the_ID();
            $cp_link = str_replace('&', '&amp;',add_query_arg( array('action' => 'yith-woocompare-add-product','id' => $id )));
            $html = '<a href="'.esc_url($cp_link).'" title="'.esc_attr($title).'" class="product-compare compare compare-link '.esc_attr($class).'" data-product_id="'.get_the_ID().'">'.$icon.'<span>'.$text.'</span></a>';
        }
        return $html;
    }
}
if(!function_exists('s7upf_wishlist_url')){
    function s7upf_wishlist_url($icon='',$text='',$class='black',$title = ''){
        $html = '';
        if(empty($text)) $text = esc_html__("Wishlist","druck");
        if(empty($icon)) $icon = '<i class="la la-heart-o"></i>';
        if(class_exists('YITH_WCWL_Init')) $html = '<a href="'.esc_url(str_replace('&', '&amp;',add_query_arg( 'add_to_wishlist', get_the_ID() ))).'" class="add_to_wishlist wishlist-link '.esc_attr($class).'" rel="nofollow" data-product-id="'.get_the_ID().'" data-product-title="'.esc_attr(the_title_attribute(array('echo'=>false))).'" title="'.esc_attr($title).'">'.$icon.'<span>'.$text.'</span></a>';
        return $html;
    }
}
if(!function_exists('s7upf_default_icon_lib')){
    function s7upf_default_icon_lib(){
        $lib = s7upf_get_option('s7upf_icon_lib','lineawesome');
        return $lib;
    }
}
    ...
    function s7upf_login_form($redirect_to = ''){
        ...
        <div class="login-form popup-form active">
            <div class="form-header">
                <h2><?php esc_html_e( 'Log In','druck' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Become a part of our community!','druck' ); ?></div>
                <div class="message ms-done ms-default"><?php esc_html_e( 'Registration complete. Please check your email.','druck' ); ?></div>
            </div>
            <form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>" method="post">
                <?php do_action( 'woocommerce_login_form_start' ); ?>
                <div class="form-field">
                    <input type="text" name="log" id="user_login" class="input" size="20" autocomplete="off"/>
                    <label for="user_login"><?php esc_html_e( 'Username or Email Address','druck' ); ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <div class="form-field">
                    <input type="password" name="pwd" id="user_pass" class="input" value="" size="20" autocomplete="off"/>
                    <label for="user_pass"><?php esc_html_e( 'Password','druck' ); ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <div class="extra-field">
                    <?php 
						if(class_exists("woocommerce")){
							...
							do_action( 'login_form','druck' );
						}
					?>
                </div>
                <div class="forgetmenot">
                    <input name="rememberme" type="checkbox" id="remembermep" value="forever" />
                    <label class="rememberme" for="remembermep"><?php esc_html_e( 'Remember Me','druck' ); ?></label>
                </div>
                <div class="submit">
                    <input type="submit" name="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Log In','druck'); ?>" />
            ...
            <div class="nav-form">
                <?php if ( get_option( 'users_can_register' ) ) :
                    echo '<a href="#registerform" class="popup-redirect register-link">'.esc_html__("Register","druck").'</a>';
                endif;
                echo '<a href="#lostpasswordform" class="popup-redirect lostpass-link">'.esc_html__("Lost your password?","druck").'</a>';
                ?>
            </div>
        </div>
        <?php
...
if(!function_exists('s7upf_register_form')){   
    function s7upf_register_form($redirect_to = ''){
        ...
        <div class="register-form popup-form">
            <div class="form-header">
                <h2><?php esc_html_e( 'Create an account','druck' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Welcome! Register for an account','druck' ); ?></div>
                <div class="message login_error ms-error ms-default"><?php esc_html_e( 'The user name or email address isn’t correct.','druck' ); ?></div>
            ...
            <form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
                ...
                <div class="form-field">
                    ...
                    <label for="user_login"><?php esc_html_e('Username','druck') ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <div class="form-field">
                    <input type="email" name="user_email" id="user_email" class="input" value="" size="25" autocomplete="off"/>
                    <label for="user_email"><?php esc_html_e('Email','druck') ?></label>
                    <div class="input-focus-line"></div>
                </div>
                <?php if ( 'no' === get_option( 'woocommerce_registration_generate_password' ) ){ ?>
                    <div class="form-field">
                        <input type="password" name="password" id="reg_passwordp" autocomplete="new-password" />
                        <label for="reg_passwordp"><?php esc_html_e( 'Password', 'druck' ); ?></label>
                        <div class="input-focus-line"></div>
                    </div>
                <?php }?>
                <div class="extra-field">
                ...
                <?php if ( 'no' != get_option( 'woocommerce_registration_generate_password' ) ){ ?>
                    <div id="reg_passmail">
                        <?php esc_html_e( 'Registration confirmation will be emailed to you.','druck' ); ?>
                    </div>
                <?php }?>
                <div class="submit"><input type="submit" name="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Register',"druck"); ?>" /></div>
                <?php do_action( 'woocommerce_register_form_end' ); ?>
            </form>

            <div class="nav-form">
                <a href="#loginform" class="popup-redirect login-link"><?php esc_html_e( 'Log in','druck' ); ?></a>
                <a href="#lostpasswordform" class="popup-redirect lostpass-link"><?php esc_html_e( 'Lost your password?','druck' ); ?></a>
            </div>
        </div>
        <?php
    }
...
if(!function_exists('s7upf_lostpass_form')){   
    function s7upf_lostpass_form($redirect_to = ''){
        ...
        <div class="lostpass-form popup-form">
            <div class="form-header">
                <h2><?php esc_html_e( 'Reset password','druck' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Recover your password','druck' ); ?></div>
                <div class="message ms-default ms-done"><?php esc_html_e( 'Password reset email has been sent.','druck' ); ?></div>
                <div class="message login_error ms-error ms-default"><?php esc_html_e( 'The email could not be sent.
Possible reason: your host may have disabled the mail function.','druck' ); ?></div>
            </div>
            <form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
                ...
                <div class="submit"><input type="submit" name="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Get New Password','druck'); ?>" /></div>
                <div class="desc note"><?php esc_html_e( 'A password will be e-mailed to you.','druck' ); ?></div>
            </form>
...
/***************************************Add Theme Function***************************************/
//Repale Count Span
add_filter('wp_list_categories', 's7upf_cat_count_span');

function s7upf_cat_count_span($output) {
  $output = str_replace('(', '<span class="count">(', $output);
...
add_action( 'init', 's7upf_modify_posts_per_page', 0);

function s7upf_modify_posts_per_page() {

	add_filter( 'option_posts_per_page', 's7upf_option_posts_per_page' );

}

function s7upf_option_posts_per_page( $value ) {

global $option_posts_per_page;

if ( is_tax( 'portfolio_cat') || is_post_type_archive('portfolio')) {
...
function s7upf_custom_post_author_archive($query) {
    if ($query->is_author){
        $query->set( 'post_type', array('portfolio','product','post') );
	}
    remove_action( 'pre_get_posts', 's7upf_custom_post_author_archive' );
}
add_action('pre_get_posts', 's7upf_custom_post_author_archive');
//Function render css inline
if(!function_exists('s7upf_get_attribute_html')){
	function s7upf_get_attribute_html($attr = 'style', $value){
		$render = $attr. '="'.$value.'"';
		return $render;
	}
}
//Fix Validated Contact Form 7
