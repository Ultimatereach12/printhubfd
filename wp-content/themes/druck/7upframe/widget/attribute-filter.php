...
if(!class_exists('S7upf_Attribute_Filter') && class_exists("woocommerce")){
    class S7upf_Attribute_Filter extends WP_Widget {
        ...
        {
            s7upf_reg_widget( 'S7upf_Attribute_Filter' );
        }

        function __construct() {
            // Instantiate the parent object
            parent::__construct( false, esc_html__('7uptheme Attribute Filter','druck'),
                array( 'description' => esc_html__( 'Filter product shop page', 'druck' ), ));
					...
					if(!empty($attr->attribute_type)){
						switch ($attr->attribute_type){
							case 'image':?>
								...
								break;
							case 'color': ?>
								<ul class="list-attr-filter tawcvs-swatches attribute-type-<?php echo esc_attr($attr->attribute_type); ?>">
									<?php 
									if(is_array($terms)){
										foreach ($terms as $term){
											if(is_object($term)){
												...
												if(!empty($color)){
													...
													echo sprintf(
														'<li><a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-color '.$class_white_color.' swatch-%s %s" title="%s" href="%s"><span class="border-rect"  '.s7upf_add_html_attr('border-color:'.$color).'></span>%s</a><span class="count">(%s)</span></li>',
														esc_attr($attribute),
														esc_attr( $term->slug ),
														esc_attr( $term->slug ),
														$selected,
									...
									if(is_array($terms)){
										foreach ($terms as $term){
											if(is_object($term)){
												...
												if(!empty($label)){
													echo sprintf(
														'<li><a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-label swatch-%s %s" title="%s" data-value="%s" href="%s"><span class="border-rect"></span>%s</a><span class="count">(%s)</span></li>',
														esc_attr($attribute),
														esc_attr( $term->slug ),
														esc_attr( $term->slug ),
														$selected,
									...
									foreach ($terms as $term) {
										if(is_object($term)){
											...
											echo    '<li class="'.esc_attr($term->slug).'-inline">
														...
														<span class="border-rect"></span>'.$term->name.'
														</a>
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Attribute:' ,'druck'); ?></label></br>
                <select id="<?php echo esc_attr($this->get_field_id( 'attribute' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'attribute' )); ?>">
                    ...
                    else echo esc_html__("No any attribute.","druck");
                    ?>
                </select>                
            </p>
        <?php
