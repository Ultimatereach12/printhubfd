...
if(!function_exists('s7upf_vc_search_form')){
    function s7upf_vc_search_form($attr){
        ...
        $data_array = array_merge(array(
            'style'         => '',
            'placeholder'   => esc_html__("Search","druck"),
            'live_search'   => 'on',
            'search_in'     => '',
            'show_cat'      => 'on',
            'cats_product'  => '',
...
if ( ! function_exists( 's7upf_add_admin_search' ) ) {
    function s7upf_add_admin_search(){
        vc_map( array(
            "name"          => esc_html__("Search Form", 'druck'),
            "base"          => "s7upf_search_form",
            "icon"          => "icon-st",
            "category"      => esc_html__("7UP-Elements", 'druck'),
            "description"   => esc_html__('Display search form', 'druck' ),
            "params"        => array(
                array(
                    "type"          => "dropdown",
                    "admin_label"   => true,
                ...
                array(
                    "type"          => "dropdown",
                    "admin_label"   => true,
                    "heading"       => esc_html__("Live Search",'druck'),
                ...
                array(
                    "type"          => "textfield",
                    "admin_label"   => true,
                    "heading"       => esc_html__("Place holder input",'druck'),
                    "param_name"    => "placeholder",
                ),
                array(
                    "type"          => "dropdown",
                    "heading"       => esc_html__("Search in post type",'druck'),
                    "param_name"    => "search_in",
                    "value"         => array(
                        esc_html__("All",'druck')       => '',
                        esc_html__("Post",'druck')      => 'post',
                        esc_html__("Product",'druck')   => 'product',
                        )
                ),
                array(
                    "type"          => "dropdown",
                    "admin_label"   => true,
                    "heading"       => esc_html__("Show categories dropdown",'druck'),
                    "param_name"    => "show_cat",
                    "value"         => array(
                        esc_html__("On",'druck')    => 'on',
                        esc_html__("Off",'druck')   => 'off',
...
if(!function_exists('s7upf_live_search')){
    function s7upf_live_search() {
        ...
        if( $query->have_posts() && !empty($key) && !empty($trim_key)){
            ...
            echo '<p class="text-center">'.esc_html__("No any results with this keyword.","druck").'</p>';
        }
        wp_reset_postdata();
    }
}