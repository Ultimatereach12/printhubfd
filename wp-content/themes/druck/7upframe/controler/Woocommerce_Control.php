...
if(class_exists("woocommerce")){
        ...
        function s7upf_product_popup_content() {
            ...
            if( $query->have_posts() ):
                ...
                while ( $query->have_posts() ) : $query->the_post();    
                    $style = '';
                    s7upf_get_template_woocommerce('single-product/popup',$style,false,true);
                endwhile;
                echo '</div></div>';
            endif;
            wp_reset_postdata();
	...
	add_action( 'woocommerce_single_product_summary','s7upf_timer_countdown',10 );
    add_action( 'woocommerce_single_product_summary', 's7upf_product_guide', 35 );
    ...
    add_action('s7upf_extra_attributes', 'woocommerce_template_single_add_to_cart',10);
    ...
    if(!function_exists('s7upf_custom_short_description')){
		function s7upf_custom_short_description( $des ) {
			$des = str_replace('<p>', '<p class="product-desc desc">', $des);
            $show_des = s7upf_get_option('show_excerpt','on');
            if($show_des == 'on' && $des) return $des;
        }
    }
    ...
    if(!function_exists('s7upf_shop_wrap_before')){
        function s7upf_shop_wrap_before(){
            ...
            $style          = s7upf_get_option('shop_default_style','list');
			switch($style){
				case 'grid-2-col':
					$column = 2;
					$style = 'grid';
            ...
            ?>
            <div class="product-<?php echo esc_attr($style)?>-view product-<?php echo esc_attr($style)?>-view-<?php echo esc_attr($column);?>-column <?php echo esc_attr($grid_type.' '.$gap_product)?> products-wrap js-content-wrap" data-load="<?php echo esc_attr($data_ajax)?>">
    ...
    if(!function_exists('s7upf_woocommerce_pagination')){
        function s7upf_woocommerce_pagination(){
            ...
            if($shop_style == 'load-more' && $max_page > 1){
                ...
                echo    '<div class="btn-loadmore large-button">
                            <a href="#" class="product-loadmore shop-button bg-color2" 
    ...
    if(!function_exists('s7upf_woocommerce_thumbnail_loop')){
        function s7upf_woocommerce_thumbnail_loop($size,$animation = '',$echo = true) {
            $img_hover_html = '';
            if($animation == 'rotate-thumb' || $animation == 'zoomout-thumb' || $animation == 'translate-thumb' || $animation == 'fade-thumb') {
    ...
    if(!function_exists('s7upf_product_quickview')){
        function s7upf_product_quickview($icon = '<i class="la la-arrows"></i>',$title='',$class = '',$echo = true) {
            $html = '<a title="'.esc_attr__("Quick View","druck").'" data-product-id="'.get_the_id().'" href="'.esc_url(get_the_permalink()).'" class="product-quick-view quickview-link '.esc_attr($class).'">'.$icon.' '.$title.'</a>';
        ...
        function s7upf_product_label($echo = true) {
            ...
            if($product->is_on_sale()){
                ...
                if($from){
                    ...
                    $html .= apply_filters( 'woocommerce_sale_flash','<span class="sale pull-right transition">'.esc_html__("sale","druck").'</span>');
                }
            }
            if($uppsell < 0) $html .=   '<span class="new pull-left transition">'.esc_html__("new","druck").'</span>';
        ...
        function s7upf_addtocart_link($echo = true,$style = '',$el_class = ''){
            ...
            if ( $product ) {                
                switch ($style) {
                    ...
                    default:
                        $icon = '<i class="la la-cart-plus"></i>';
                        $text = '<span class="title14 white bg-color">'.$product->add_to_cart_text().'</span>';
                        $title = '';
						$btn_class = 'addcart-link '.$el_class;                
                        break;
                }
                ...
                $button_html =  apply_filters( 'woocommerce_loop_add_to_cart_link',
                    sprintf( '<a href="%s" title="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="%s product_type_%s">'.$icon.'%s</a>',
    ...
    if ( !function_exists( 's7upf_catalog_ordering' ) ) {
        function s7upf_catalog_ordering($query,$set_orderby = '',$list_item = false,$add_class = '') {        
            ...
            $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
                'menu_order' => esc_html__( 'Default sorting', 'druck' ),
                'popularity' => esc_html__( 'Sort by popularity', 'druck' ),
                'rating'     => esc_html__( 'Sort by average rating', 'druck' ),
                'date'       => esc_html__( 'Sort by newness', 'druck' ),
                'price'      => esc_html__( 'Sort by price: low to high', 'druck' ),
                'price-desc' => esc_html__( 'Sort by price: high to low', 'druck' )
            ) );
            ...
            if ( wc_get_loop_prop( 'is_search' ) ) {
                $catalog_orderby_options = array_merge( array( 'relevance' => esc_html__( 'Relevance', 'druck' ) ), $catalog_orderby_options );
        ...
        function s7upf_load_product_filter() {
            ...
            if(isset($pagination) && !empty($pagination)){?>
                <div class="products row list-product-wrap js-content-main">
                    ...
                    else echo '<div class="filter-noresult-wrap"><div class="filter-noresult title18 text-center">'.esc_html__("No result found with current filter value.","druck").'</div></div>';
                    ?>
                </div>
                <?php
    ...
    if(!function_exists('s7upf_show_single_product_data')){
        function s7upf_show_single_product_data(){
            ...
            $item_res   = s7upf_get_value_by_id('show_single_itemres');
			if(empty($item_res)) $item_res   = s7upf_get_option('show_single_itemres','0:1,480:2,990:3,1200:4');
	...
	if(!function_exists('s7upf_product_guide')){
        function s7upf_product_guide(){            
            s7upf_get_template_woocommerce('single-product/guide','',false,true);
        }
    }
	/*************************************************************
							Add Funtion
	**************************************************************/
	//Timer Count Down
	if(!function_exists('s7upf_timer_countdown')){
		function s7upf_timer_countdown(){
	...
	//Product Progress
	if ( ! function_exists( 's7upf_percent_sale' ) ) {
		function s7upf_percent_sale($style=''){
			global $product;
			$from = $product->get_regular_price();
			$to = $product->get_price();
			$percent = $html =  '';
			if($from != $to && $from > 0){
				$percent = round(($from-$to)/$from*100);            
				$html .=   '<div class="percent-sale">
								<span class="title24">-'.esc_html($percent).'% </span>
							</div>';
			}
			return $html;
		}
	}
	//Change WooCommerce Orderby
	add_filter('woocommerce_catalog_orderby', 's7upf_customize_product_sorting');
	if(!function_exists('s7upf_customize_product_sorting')){
		function s7upf_customize_product_sorting($sorting_options){
			$sorting_options = array(
				'menu_order' => esc_html__( 'Default Sorting', 'druck' ),
				'popularity' => esc_html__( 'Popularity', 'druck' ),
				'rating'     => esc_html__( 'Average Rating', 'druck' ),
				'date'       => esc_html__( 'Newness', 'druck' ),
				'price'      => esc_html__( 'Price: Low to High', 'druck' ),
				'price-desc' => esc_html__( 'Price: High to Low', 'druck' ),
			);
	...
	add_filter( 'woocommerce_product_tabs', 's7upf_woo_rename_tabs', 98 );
	function s7upf_woo_rename_tabs( $tabs ) {
		if ( isset( $tabs['additional_information']['title'])){
			$tabs['additional_information']['title'] = esc_html__( 'Information','druck' );
		}
		return $tabs;
	}
	//Product Category
	if(!function_exists('s7upf_get_product_category')){
        function s7upf_get_product_category($cat_class = ''){     
			$cat_html = '';
            $product_cats = wp_get_post_terms( get_the_id(), 'product_cat' );
			if(!empty($product_cats)){
				$max = sizeof($product_cats)-1;
				$index = rand(0,$max);
				$cat_html = '<a href="'.esc_url(get_term_link($product_cats[$index]->term_id)).'" class="cat-parent '.esc_attr($cat_class).'">'.esc_html($product_cats[$index]->name).'</a>';
			}
			return apply_filters('s7upf_output_content',$cat_html);
        }
    }    
}
