...
if(!class_exists('S7upf_BaseController')){
    class S7upf_BaseController{
        ...
        static function _add_scripts(){
            ...
            wp_enqueue_script( 'countdown',$js_url.'lib/jquery.countdown.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'waypoints',$js_url.'lib/jquery.waypoints.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'countup',$js_url.'lib/jquery.countup.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'modernizr',$js_url.'lib/modernizr.custom.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'classie',$js_url.'lib/classie.min.js',array('jquery'),null,true);
            ...
            wp_enqueue_style('jquery.fancybox-buttons',$css_url.'lib/jquery.fancybox-buttons.min.css');
            ...
            wp_enqueue_style('slick',$css_url.'lib/slick.min.css');
                ...
                foreach($add_sidebars as $sidebar){
                    if(!empty($sidebar['title'])){
                        ...
                        $custom_add_sidebar = array(
                                'name' => $sidebar['title'],
                                'id' => $id,
                                'description' => esc_html__( 'SideBar created by add sidebar in theme options.', 'druck'),
        ...
        static function s7upf_product_cat_metabox_add($tag) { 
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content Before','druck'); ?></label>
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content After','druck'); ?></label>
                <div class="wrap-metabox">
        ...
        if($s7upf_woo_widgets != 'true' && isset($active_widgets['woocommerce-sidebar']) && empty($active_widgets['woocommerce-sidebar'])){
            ...
            $widgets = array(
                'woocommerce_product_categories' => array(
                    'title' => esc_html__('Product categories','druck'),
                    'orderby' => 'name',
                    'dropdown' => 0,
                    'count' => 0,
                    'hierarchical' => 1,
                    ...
                    'title' => esc_html__('Filter by price','druck'),
                    ),
                'woocommerce_products' => array(
                    'title' => esc_html__('Products','druck'),
                    'number' =>  5,
                    'show' => '',
                    'orderby' => 'date',
                    'order' => 'desc',
