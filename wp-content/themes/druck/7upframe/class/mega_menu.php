<?php
global $s7upf_config;
if($s7upf_config['mega_menu'] == '1'){
    class S7upf_CustomMenu
    {
        static $allFields;
        static function init()
        {
            self::$allFields=array(
                'enable_megamenu'=>array(
                    'label'=>esc_html__('Enable Mega menu','druck'),
                    'type'=>'checkbox',
                    'depth'=>0,
                ),
                'icon_menu2'=>array(
                    'label'=>esc_html__('Custom width Mega menu. Example "500px".','druck'),
                    'type'=>'text',
                ),
                'image'=>array(
                    'label'=>esc_html__('Image Preview','druck'),
                    'type'=>'image',
                ),
                'content2'=>array(
                    'label'=>esc_html__('Content From Mega Item','druck'),
                    'type'=>'select',
                    'depth'=>0,
                    'choices'=> s7upf_list_post_type('s7upf_mega_item',false)
                ),
        ...
        static function _field_type_image($item,$key,$value,$d=0){
            ...
            <p class="field-custom description description-wide">
                ...
                <div class="wrap-metabox">
                    ...
                    <a class="button button-primary sv-button-remove"> <?php esc_html_e("Remove","druck")?></a>
                    <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload","druck")?></a>
    ...
    S7upf_CustomMenu::init();
    class S7upf_WalkerNavMenu extends Walker_Nav_Menu
    {
        /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)' ,'druck'), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)','druck'), $item->title );
            }
            ...
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo s7upf_add_html_attr($submenu_text); ?>><?php esc_html_e( 'sub item' ,'druck'); ?></span></span>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','druck'); ?>">&#8593;</abbr></a>
                                |
                                <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                                ...
                                ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','druck'); ?>">&#8595;</abbr></a>
                            </span>
                            <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','druck'); ?>" href="<?php
                            ...
                            ?>"><?php esc_html_e( 'Edit Menu Item' ,'druck'); ?></a>
                        </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Navigation Label' ,'druck'); ?><br />
                        <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                    </label>
                </p>
                <p class="description description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Title Attribute' ,'druck'); ?><br />
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Open link in a new window/tab','druck' ); ?>
                    </label>
                </p>
                <?php

                S7upf_CustomMenu::AdminAddFields($item,$depth);

                ?>
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'CSS Classes (optional)' ,'druck'); ?><br />
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Link Relationship (XFN)' ,'druck'); ?><br />
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Description' ,'druck'); ?><br />
                        <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                        <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.' ,'druck'); ?></span>
                    </label>
                </p>

                <p class="field-move hide-if-no-js description description-wide">
                    <label>
                        <span><?php esc_html_e( 'Move' ,'druck'); ?></span>
                        <a href="#" class="menus-move-up"><?php esc_html_e( 'Up one' ,'druck'); ?></a>
                        <a href="#" class="menus-move-down"><?php esc_html_e( 'Down one' ,'druck' ); ?></a>
                        <a href="#" class="menus-move-left"></a>
                        <a href="#" class="menus-move-right"></a>
                        <a href="#" class="menus-move-top"><?php esc_html_e( 'To the top' ,'druck'); ?></a>
                    </label>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s','druck'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e( 'Remove' ,'druck'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel','druck'); ?></a>
                </div>
